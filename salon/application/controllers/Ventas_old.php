				...
				if ($data_ingreso != NULL) {
					...
					foreach ($data_ingreso as $fila) {
						...
						$form_data = array(
							...
							'garantia' 			=> $this->ventas->getGarantia(
								$fila['id_producto'],
								$fila['est']
							)
						);
						...
						// todo: remove comment
						// $stock_data = $this->ventas->get_stock($fila['id_producto'], $color, $id_sucursal);
						// $newstock = ($stock_data->cantidad) - $cantidad;
						// if ($tipo == 0) {
						// 	$this->utils->update("stock", array('cantidad' => $newstock,), "id_stock=" . $stock_data->id_stock);
						// }

						if ($this->ventas->has_imei_required($fila['id_producto'])) {
							// code...
							$imei_required = true;
						}
		...
		if ($this->input->method(TRUE) == "POST") {
			...
			if ($id_venta == -1) {
				...
				$data = array(
					...
					'indicaciones'         => "FINALIZADA REF cor 0",
					'id_cliente'           => $id_cliente,
					'id_estado'            => 1,
					'id_sucursal_despacho' => $id_sucursal,
					'correlativo'          => 0,
		...
		if ($this->input->method(TRUE) == "POST") {
			...
			switch ($tipodoc) {
				case 1:
					...
					$correlativo = $this->ventas->get_correlative('tik', $id_sucursal);
					$this->ventas->update_correlative('tik', $correlativo, $id_sucursal);break;
				case 2:
					...
					$correlativo = $this->ventas->get_correlative('cof', $id_sucursal);
					$this->ventas->update_correlative('cof', $correlativo, $id_sucursal);	break;
				case 3:
					$nit = $this->input->post("nit");
					$nrc = $this->input->post("nrc");
					$form_data = array(
