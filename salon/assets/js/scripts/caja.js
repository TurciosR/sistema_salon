...
$("#caja").select2();
  ...
  $("#form_add_apert").on('submit', function(e){
    e.preventDefault();
    $(this).parsley().validate();
    if ($(this).parsley().isValid()){
      $("#btn_add_apert").prop("disabled",true)
      save_data_apert();
    }
  });
...
$(document).on('change', '#caja', function(event) {
  $("#btn_add_apert").removeAttr("disabled");
});
function save_data_apert(){
  $("#divh").show();
	$("#main_view").hide();

  var errors = false;
  var cuantos = 0;
  var data = $('#caja').select2('data');
  var error_array = [];
  if(data) {
    if(data[0].id>0){
      errors = false;
      ...
      errors = true;
        error_array.push('No ha seleccionado caja para Apertura');
    }
  }
  if (errors==false) {
    let form = $("#form_add_apert");
    let formdata = false;
    if (window.FormData) {
      formdata = new FormData(form[0]);
    }
