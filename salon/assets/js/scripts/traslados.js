...
$(document).ready(function () {
  ...
  // Know if just one Branch(sucursal) exist, if so, then block the button
  // "Nuevo traslado" and display a message warning.
  if ($("#sucursales").length) {
    if ($("#sucursales").data("total-suc") < 2) {
      $("a.btn.btn-success").attr('href', base_url);
      $("a.btn.btn-success").text('Regresar al Inicio');
      $("a.btn.btn-success").attr('class','btn btn-secondary');
      $(`<div class='alert alert-info' role='alert'>
        No puede hacer traslados. Solo posee una Sucursal, consulte con su
        administrador y/o proveedor para agregar más Sucursales.
      </div>`).appendTo($(".ibox-title"));
    }
  }
